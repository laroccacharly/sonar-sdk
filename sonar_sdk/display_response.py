...
def save_response_to_file(response: ChatCompletion):
    """
    Save the formatted response to a file in the directory specified by SEARCH_HISTORY_PATH
    environment variable, or 'responses' folder if not set.
    
    Args:
        response: The ChatCompletion response from the API
    """
    # Check if SEARCH_HISTORY_PATH environment variable is set
    search_history_path = os.environ.get("SEARCH_HISTORY_PATH")
    
    if search_history_path:
        # Use the path from environment variable
        responses_dir = Path(search_history_path)
    else:
        # Default to 'responses' directory
